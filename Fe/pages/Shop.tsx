...
const Shop: React.FC<ShopProps> = ({ onAddToCart }) => {
  ...
  const ProductSkeleton: React.FC<{ viewMode: "grid" | "list" }> = ({
    viewMode,
  }) => {
    if (viewMode === "list") {
      return (
  ...
  const allCategories = [
    ...new Set(products.map((p) => p.category.name.toUpperCase())),
  ].sort();

  return (
    <div className="animate-in fade-in slide-in-from-bottom-4 duration-700">
      {/* Page Hero */}
      <section className="relative h-[40vh] md:h-[70vh] flex items-center justify-center bg-slate-900 text-white text-center overflow-hidden">
        ...
        <div className="relative z-10 max-w-4xl px-4">
          <h1 className="text-4xl md:text-7xl font-bold tracking-tight mb-4">
            {lang === "en" ? "The Catalog" : "Katalog Utama"}
          </h1>
          <div className="w-20 h-1 bg-emerald-500 mx-auto rounded-full mb-6" />
          <p className="text-slate-300 text-sm md:text-xl font-light tracking-wide uppercase">
            {lang === "en"
              ? "Curated Perfection in Every Detail"
              : "Kesempurnaan Terkurasi di Setiap Detail"}
          </p>
        </div>
      </section>
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 md:gap-8 mb-8 md:mb-12">
          <div>
            ...
            <p className="text-slate-500 mt-1 md:mt-2 text-sm md:text-base">
              Showing {!loading ? products.length : "..."} premium selections
            </p>
          </div>
        ...
        <div className="flex flex-col lg:flex-row gap-8 md:gap-12">
          ...
          <aside className="w-full lg:w-64 space-y-8 md:space-y-10">
            <div className="space-y-4 md:space-y-6">
              <h3 className="text-[10px] md:text-xs font-bold uppercase tracking-widest text-slate-900">
                Categories
              </h3>
            ...
            <div className="p-6 bg-slate-900 rounded-[2rem] text-white space-y-3 md:space-y-4 hidden lg:block">
              ...
              <p className="text-[11px] text-slate-400 leading-relaxed">
                Visit our support center for assistance with orders, payments,
                or account issues.
              </p>

              <Link
                to="/faq"
                className="block w-full text-center bg-white text-slate-900 py-2 md:py-2.5 rounded-lg text-xs md:text-sm font-bold hover:bg-emerald-500 hover:text-white transition-colors"
              >
                Go to Support
              </Link>
            </div>
                ...
                {products.length > 0 ? (
                  ...
                  >
                    {products.map((product) => (
                      <div
                        key={product.id}
                        className={`group bg-white rounded-2xl md:rounded-[2rem] overflow-hidden shadow-sm hover:shadow-xl transition-all duration-500 ${
                          viewMode === "list"
                            ? "flex flex-col sm:flex-row gap-4 md:gap-8"
                            : ""
                        }`}
                      >
                        <Link
                        ...
                        >
                          <div className="mb-4">
                            <p className="text-[9px] md:text-[10px] text-emerald-600 font-bold uppercase mb-1">
                          ...
                          <p className="text-xl md:text-2xl font-bold text-slate-900 mb-5 md:mb-6">
