...
const ProductDetail: React.FC<ProductDetailProps> = ({ onAddToCart }) => {
  ...
  const {
    ...
    enabled: !!slug, // penting
    queryFn: async () => {
      const response = await api.get(`/products/${slug}`);
      return response.data.products; // Fallback jika data langsung di root
    },
  });
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 md:gap-16">
        ...
        <div className="space-y-4 md:space-y-6">
          ...
          <div className="grid grid-cols-4 gap-2 md:gap-4">
            {[1, 2, 3, 4].map((i) => (
              <div
                key={i}
                className="aspect-square rounded-xl md:rounded-2xl bg-slate-100 overflow-hidden cursor-pointer hover:ring-2 hover:ring-emerald-500 transition-all"
              >
                <img
        ...
        <div className="space-y-8 md:space-y-10">
          <div>
            ...
            <p className="text-2xl md:text-3xl font-bold text-slate-900 mt-4 md:mt-6">
          ...
          <div className="space-y-4 md:space-y-6">
            <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-4 md:gap-6">
              ...
              <button onClick={() => handleToggleFavorite(product.slug)}className="hidden sm:block p-4 md:p-5 border border-slate-200 rounded-xl md:rounded-2xl hover:bg-slate-50 transition-colors">
            ...
            {product.stock < 10 && (
              <p className="text-red-500 text-xs md:text-sm font-bold flex items-center gap-2">
                <Shield className="w-3.5 h-3.5 md:w-4 h-4" /> Only{" "}
          ...
          <div className="bg-emerald-50 rounded-2xl md:rounded-3xl p-6 md:p-8 space-y-3 md:space-y-4 border border-emerald-100">
            ...
            <p className="text-slate-600 text-xs md:text-sm">
              Have questions about this item? Ask our concierge.
            </p>
            <form onSubmit={handleAiChat} className="flex gap-2">
              ...
              >
                {isAiLoading ? (
                  <RefreshCcw className="w-4 h-4 md:w-5 h-5 animate-spin" />
                ) : (
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6 pt-8 md:pt-10 border-t border-slate-100">
            <div className="flex flex-col items-center text-center p-3 md:p-4">
              ...
              <span className="text-[9px] md:text-[10px] text-slate-500 uppercase">
                On orders over $500
              </span>
            </div>
            <div className="flex flex-col items-center text-center p-3 md:p-4">
      ...
      <div className="mt-24 md:mt-32">
        <div className="flex gap-6 md:gap-12 border-b border-slate-100 mb-8 md:mb-10 overflow-x-auto whitespace-nowrap scrollbar-hide">
          <button
            ...
            className={`pb-4 text-[10px] md:text-sm font-bold uppercase tracking-widest transition-colors relative ${
        ...
        <div className="max-w-3xl animate-in fade-in slide-in-from-bottom-2">
          ...
          {activeTab === "specs" && (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-y-4 md:gap-y-6 gap-x-12 md:gap-x-20">
              {/* SKU (opsional) */}
              {product.sku && (
                <div className="flex justify-between py-3 md:py-4 border-b border-slate-50 text-sm md:text-base">
              ...
              {product.product_spec?.map((spec, index) => (
                <div
                  key={index}
                  className="flex justify-between py-3 md:py-4 border-b border-slate-50 text-sm md:text-base"
                >
            ...
            <div className="space-y-10 md:space-y-12">
              {product.review?.length > 0 ? (
                product.review.map((review) => (
                  <div key={review.id} className="space-y-3 md:space-y-4">
                    {/* Header */}
                    <div className="flex justify-between items-center">
                      <div className="flex items-center gap-3 md:gap-4">
                        <div className="w-10 h-10 md:w-12 md:h-12 bg-slate-100 rounded-full flex items-center justify-center text-slate-900 font-bold text-sm">
                          U{review.user_id}
                        </div>
                        <div>
                          ...
                          <p className="text-[9px] md:text-[10px] text-slate-400 uppercase tracking-tighter">
                            Verified Purchase
                          </p>
                        </div>
                      </div>
                    ...
                    {/* Comment */}
                    <p className="text-slate-600 leading-relaxed font-light italic text-sm md:text-base">
                      “{review.comment}”
                    </p>
                  </div>
                ))
              ...
              {/* Footer */}
              <button className="text-emerald-600 font-bold hover:underline text-sm md:text-base">
                Read All {product.review?.length ?? 0} Reviews
              </button>
            </div>
          )}
        </div>
